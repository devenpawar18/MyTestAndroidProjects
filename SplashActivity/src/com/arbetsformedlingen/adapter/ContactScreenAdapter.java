...
public class ContactScreenAdapter extends BaseAdapter {
	...
	public ContactScreenAdapter(Context context) {
	...
	class HeaderAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			try {
				...
				if (title.length() == 0) {
					view = LayoutInflater.from(context).inflate(R.layout.info_header_without_title, parent, false);
					return view;
				} else {
					view = LayoutInflater.from(context).inflate(R.layout.info_header, parent, false);
				}
				text = (TextView) view.findViewById(R.id.info_header_text);
